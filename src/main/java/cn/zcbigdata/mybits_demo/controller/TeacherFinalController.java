...
public class TeacherFinalController {
    @Autowired
    public TeacherFinalService teacherFinalService;
    ...
    public String update(HttpServletRequest request) {
        ...
        if(adminFlag.equals(session.getAttribute("admin_flag"))){
            if(IsNullUtil.IsNullTrue(statusString)) {
                ...
                TeacherFinal teacherFinal = new TeacherFinal();
                teacherFinal.setStatus(statusString);

                teacherFinalService.update(teacherFinal);
    ...
    public String selectAll(HttpServletRequest request) throws Exception {
        ...
        String data="";
        if(adminFlag.equals(session.getAttribute("admin_flag"))){
            List<TeacherFinal> listsList = teacherFinalService.selectAll(pageInteger,limitInteger,cnoString);
            if(listsList != null) {
    ...
    public String selectCount(HttpServletRequest request) {
        HttpSession session = request.getSession();
        String cnoString = (String) session.getAttribute("cno");
        int count = teacherFinalService.selectCount(cnoString);
