...
public class TeacherMiddleController {
    @Autowired
    public TeacherMiddleService teacherMiddleService;
    ...
    public String update(HttpServletRequest request) {
        ...
        if(adminFlag.equals(session.getAttribute("admin_flag"))){
            if(IsNullUtil.IsNullTrue(statusString)) {
                ...
                teacherMiddle.setStatus(statusString);

                teacherMiddleService.update(teacherMiddle);
    ...
    public String selectAll(HttpServletRequest request) throws Exception {
        ...
        String data="";
        if(adminFlag.equals(session.getAttribute("admin_flag"))){
            List<TeacherMiddle> listsList = teacherMiddleService.selectAll(pageInteger,limitInteger,cnoString);
            if(listsList != null) {
    ...
    public String selectCount(HttpServletRequest request) {
        HttpSession session = request.getSession();
        String cnoString = (String) session.getAttribute("cno");
        int count = teacherMiddleService.selectCount(cnoString);
